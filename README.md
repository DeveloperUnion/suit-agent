...
- 会話は Provider の state ではなく `localStorage` に置く（`lib/data/agent-chat.ts`）。
  接客の合間に開き直す使い方で、リロードのたびに消えると「さっき何を頼んだか」が
  追えなくなるため。顧客と同じくスタッフごとに分ける
