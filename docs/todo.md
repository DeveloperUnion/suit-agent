...
済んでいることは `docs/database.md` の「本番」を見る。残りは 5 つ。

## 検証用の顧客 600 名を消す

本番には**架空の顧客 600 名**が入っている（下平 300 / 北島 300）。実機で
一覧・検索・アプローチ・アシスタントを触るために入れたもので、**店舗のデータではない**。

```bash
supabase db query --linked -f supabase/demo/99_teardown.sql
```

最後に出る 3 つの数がすべて 0 であること。0 でなければ commit しないこと。

消えるのはこの投入で作った 600 名とその従属行、および `change_log` の該当分だけ。
手で入れた既存の顧客 9 名（山岸秀匡さん・横川尚隆さんなど）と、手で入れた語
（カレー・サーフィン・サッカー観戦・ビジネス・ワイン・保険の営業・野球観戦）は残る。

`fact_labels` はどれも消さない。この投入で増えた 27 語と、店舗が使い始める語は
あとから見分けが付かないため — 顧客が消えれば、どの顧客にも付いていない語になるだけ。

投入し直すときは `npm run db:demo-seed` で SQL を作り、`supabase/demo/` を
01 から順に流す（正は `scripts/generate-demo-seed.ts`）。
