 ...
 * やってみないと適切な長さが分からないので、店舗に開けている。
 *
 * 節目は「日数」ではなく「月」で数える。半年は 180 日ではなくお渡し日の半年後であり、
 * 顧客にとっても暦の上での区切りだから。
 */
...
/**
 * 記念日の何日前から出すか。**既定値だけ**で、実際に効く値は app_settings から来る。
 *
 * 当初は「1 週間前」を店舗として確定した決めごとと見なして定数に固定していたが、
 * 声をかける準備に要る日数は節目と同じで、やってみないと分からなかった。
 * 0 は「当日だけ出す」。画面ではカウントダウン（「あと3日」）で見せる。
 */
export const DEFAULT_ANNIVERSARY_LEAD_DAYS = 7;
