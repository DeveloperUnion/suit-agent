...
-- ── 提案の下見 ──────────────────────────────────────────
--
-- ラベルの表記を決めるのは DB。モデルに「ぼるだりんぐ」と書かれた瞬間に
-- 「ボルダリングが趣味な人」から漏れるので、寄せる判断をモデルの外に出す。

-- ラベルをまだ持っていない人（走り書きだけの人）に足す。
select is(
  app.plan_fact_add('e6c00000-0000-4000-8000-000000000010', array['クライミング'])
    -> 'labelNames',
  '["ボルダリング"]'::jsonb,
  '別名で来ても既存語の表記へ寄る（モデルの書いた文字列をそのまま使わない）'
);
select is(
  app.plan_fact_add('e6c00000-0000-4000-8000-000000000010', array['クライミング'])
    -> 'newLabelNames',
  '[]'::jsonb,
  '既存語に寄ったものは「新しい語」にしない（語彙が二重に増えない）'
);

select is(
  app.plan_fact_add('e6c00000-0000-4000-8000-000000000004', array['ボルダリング'])
    -> 'alreadyHas',
  '["ボルダリング"]'::jsonb,
  'すでに付いている語は落として「すでに入っています」と言えるようにする'
);

-- 会話から来た id でも境界は越えさせない。
select is(
